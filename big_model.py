...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.record_heatmap:
            heatmap0 = torch.zeros(
                (self.fc1.in_features,), device=x.device, dtype=torch.int64
            )
            self.heatmaps.append(heatmap0)
            heatmap1 = torch.zeros(
                (self.fc1.out_features,), device=x.device, dtype=torch.int64
            )
            self.heatmaps.append(heatmap1)
            heatmap2 = torch.zeros(
                (self.fc2.out_features,), device=x.device, dtype=torch.int64
            )
            self.heatmaps.append(heatmap2)
            heatmap3 = torch.zeros(
                (self.fc3.out_features,), device=x.device, dtype=torch.int64
            )
            self.heatmaps.append(heatmap3)
            heatmap4 = torch.zeros(
                (self.fc4.out_features,), device=x.device, dtype=torch.int64
            )
            self.heatmaps.append(heatmap4)
            heatmap5 = torch.zeros(
                (self.fc5.out_features,), device=x.device, dtype=torch.int64
            )
            self.heatmaps.append(heatmap5)
            heatmap6 = torch.zeros(
                (self.fc6.out_features,), device=x.device, dtype=torch.int64
            )
